...
/**
 * Design 页面 - 与 Editor 功能对齐
 * 布局：
 * - 左侧：DesignSidebar（tab栏固定，展开面板浮在canvas上）
 * - 中间：RenderCanvas（16:9 比例）
 * - 底部中央悬浮：ToolsBar（工具栏）
 * - 右侧：RightSider（动态宽度）
 */
const Design: React.FC = () => {
    ...
    return (
        <ConfigProvider theme={{
            ...
            <Layout className="editorThemeScope designPage" style={{ height: '100vh' }}>
                <DesignHeader style={{ height: layout.headerHeight, minHeight: layout.headerHeight }} />
                ...
                <Layout style={{
                    ...
                    {/* 渲染区域 - 16:9 比例 */}
                    <Content className="designCanvasArea" style={{
                    ...
                    {/* 右侧属性面板 - 固定宽度 */}
                    <div className="rightSider designRightSider" style={{ width: layout.rightPanelWidth }}>
                        <RightSider></RightSider>
                    </div>
                </Layout>

                {/* UVShow 视图 */}
                {isUVShowView && <UVShowInline fromDesign />}
            </Layout>
